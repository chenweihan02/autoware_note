...
// replan函数遍历　lane_array.lanes
// lane_array.lanes内的元素都是Lane类型，其中储存了节点　waypoint_loader从本地文件内读取的轨迹点数据
// 每个轨迹点记录了世界坐标系，一般为通用横轴墨卡托投影(UTM)坐标系下的坐标 x,y,z,航向角等必要信息供规划所用．
// 对每一条lane, replanner都调用replanLaneWaypointVel函数
...
void WaypointReplannerNode::publishLaneArray()
{
  ...
  // 发布重规划过的路径集合至话题　/based/lane_waypoints_array ? /lane_waypoints_array
...
// 为话题　/config/waypoint_replanner的回调函数，进行一些成员变量的初始化
